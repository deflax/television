...
- **Audio-only HLS sidecar** - One shared FFmpeg sidecar derives audio from `/live/stream.m3u8` and writes `/live/audio.m3u8` plus `/live/audio/*.ts`
...
- `/live/audio.m3u8` - Audio-only HLS media playlist generated by the shared sidecar
- `/live/audio/*.ts` - Audio-only MPEG-TS HLS segments
...
The audio-only endpoint is not an ABR variant and does not change `ABR_VARIANTS` or segment-store variant counts. The mux service runs a single service-global sidecar FFmpeg process after the primary mux produces its first segment; listeners fetch the same playlist and do not create per-user FFmpeg processes.
...
| `/live/audio.m3u8` | GET | Public | Audio-only mux stream playlist |
| `/live/audio/<file>.ts` | GET | Public | Audio-only mux stream segment |
